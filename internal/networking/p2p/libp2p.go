...
// Instance is the libp2p instance for networking usages
type Instance struct {
    ...
    peersLock sync.Mutex // For resolve synchronisation issue

    startMDNS func() error
    closeMDNS func() error
}

// NewInstance initialise a blank libp2p instance
...
// SetP2PHost using an existing libp2p host to initialise the instance
func (i *Instance) SetP2PHost(existingHost *host.Host) *Instance {
    i.host = existingHost
    return i
}

// Build construct the P2P instance using the given configuration
